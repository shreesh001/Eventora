...
const events = [
    {
        ...
        time: '10:00 AM', // Added required time field
        location: 'Silicon Valley Innovation Center, CA',
        category: 'Tech', // Changed from 'Technology' to match Enum
        totalSeats: 200,
        ticketPrice: 0,
        imageUrl: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&q=80&w=800' // Changed from 'image'
    },
    {
    ...
    },
    {
        ...
        category: 'Tech', // Changed from 'Business' to match Enum
        totalSeats: 150,
        ticketPrice: 5000,
...
const seedDatabase = async () => {
    try {
        ...
        const hashedUsers = users.map(u => ({
            ...
            verified: true // Changed from isVerified to match your schema
        }));
        ...
        for (const event of createdEvents) {
            ...
            for (const user of selectedUsers) {
                ...
                bookingsData.push({
                    user: user._id,   // Fixed: Changed from userId to user
                    event: event._id, // Fixed: Changed from eventId to event
                    numberOfSeats: 1, // Explicitly defined
                    status: status,
